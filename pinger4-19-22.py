...
#-----------------------------------------------------------------------------------------------------
def formatDump(x):
    if x.delta > minIssueTimeToDisplay:
...
def setPreferences():
    ...
    def grabTheValue():
        global minIssueTimeToDisplay
        minIssueTimeToDisplay= e1.get()

    tk3=Tk()
    Label(tk3,
             text="min timeout to display").grid(row=0)
    Label(tk3,
             text="reserved for future use").grid(row=1)
    ...
    Button(tk3, text='Quit',  command=tk3.destroy).grid(row=3, column=0,  sticky=W,  pady=4)
    Button(tk3, text='apply', command=grabTheValue).grid(row=3,  column=1, sticky=W,  pady=4)

    tk3.update_idletasks()
    tk3.update()
...
def dumpOutagesTk():
    ...
    menubar2 = Menu(tk2)
    filtermenu = Menu(menubar2, tearoff=0)
    menubar2.add_cascade(label="filter", menu=filtermenu)
    tk2.config(menu=menubar2)


    stat=("----list of outages since "+ str(initialTime).split(".", 1)[0]+ " with no response time > "+str(minIssueTimeToDisplay) +" -----\n")
    T.insert(END, stat)
...
parser.add_argument('-mttd', dest='minIssueTimeToDisplay', type=int,  help='when dumping from UI, only time out greater than this will be displayed ')
...
if args.minIssueTimeToDisplay is not None:
        minIssueTimeToDisplay = args.minIssueTimeToDisplay
else:
        minIssueTimeToDisplay = 3
if args.useTk:
        useTk= True
else:
        useTk= False
...
splash = "*** Pinger "+VERSION+" Started at " + startedAt + " on " + platform.system()+\
        ...
        " seconds.\n->Outages lasting more than "+str(issueThreshold) + " seconds will be flagged as major outage\n" \
                    "->dumps will show timeout > "+str(minIssueTimeToDisplay) +" seconds\n"
...
if useTk: #-----------------------------------set up up
    ...
    filemenu.add_command(label="Preferences", command=setPreferences)
